...
function App() {
  ...
  return <div className="shell">
    ...
    <main>
      <div className="intro"><p className="eyebrow">DEIN PROGRAMM. DEIN RHYTHMUS.</p><h1>{tab === 'radio' ? 'Platz für gute Gedanken.' : tab === 'profile' ? 'So klingt dein Tag.' : 'Hören. Sperren. Testen.'}</h1><p>{tab === 'radio' ? 'Der Anfang deines persönlichen Radios. Heute testen wir die Wiedergabe – die KI-Redaktion folgt.' : tab === 'profile' ? 'Deine Vorlieben bleiben auf diesem Gerät. Sie werden erst mit der kommenden Redaktion wirksam.' : 'Finde heraus, wie zuverlässig dein Android-Gerät im Hintergrund weiterspielt.'}</p></div>
      {tab === 'radio' && <>
        ...
        <section className="panel"><div className="section-heading"><h2>Als Nächstes</h2><span>Lokaler Audiotest</span></div><ol className="queue" key={queueVersion}>{player.tracks.map((t, i) => <li key={t.id}><button onClick={() => void player.start(i)} aria-current={state.index === i ? 'true' : undefined}><span className="queue-num">{String(i + 1).padStart(2, '0')}</span><span><strong>{t.title}</strong><small>{t.kind}</small></span><span aria-hidden="true">{state.index === i ? '●' : '↗'}</span></button></li>)}</ol></section>
        <section className="note"><strong>Ein erster Funktionstest</strong><p>Du hörst leise Testtöne, keine KI-Nachrichten. ASK, Mistral und Spotify sind noch nicht verbunden. Du kannst stattdessen eigene Audiodateien auswählen.</p></section>
      </>}
