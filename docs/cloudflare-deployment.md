...
The private app now offers a mobile-friendly form for one source and plays the returned MP3 in the existing player. The verifier asks ASK to enumerate factual claims and supply direct supporting quotes; the Worker requires each quoted excerpt to match the supplied source text and each source ID to be cited by the script. This is an LLM-assisted check, not a guarantee of truth or a substitute for a curated source-ingestion service. The current slice accepts one source submitted manually; it does not yet fetch feeds. The in-flight duplicate guard is isolate-local; persistent daily request and character limits cap provider use, but completed-request audio is not durably cached.
